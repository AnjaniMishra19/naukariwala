...
def phone_register(request):
    try:
        if request.method == 'POST':
            ...
            elif CandidateData.objects.filter(phone_no=phone).exists():
...
def resend_otp(request):
    try:
        if request.method == 'POST':
            ...
            if CandidateData.objects.filter(user_id=user_id).exists():
                send_otp(phone, user_id)
                response = {"status":200, "data":phone, "message":"OTP resent to your phone number "+ str(phone)+"."}
            else:
...
def verify_otp(request):
    try:
        if request.method == "POST":
            ...
            user_data = CandidateData.objects.get(phone_no=phone)
            if user_data.otp_validity > datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"):
                if user_data.otp == otp:
                    CandidateData.objects.filter(user_id=user_id).update(otp="",verified=True,otp_validity="", token=token)
...
def candidate_register(request):
    try:
        if request.method == 'POST':
            ...
            #Email already exist 
            if CandidateData.objects.exists(user_id=decoded_token['user_id']):
                if CandidateData.objects.filter(email=email , status=1):
                ...
                # email_sent = email_notification(email,data["password"],first_name)
                    ...
                    if formType=="metaData":
                        CandidateData.objects.filter(user_id=user_id).update(
                        ...
                        response = {"status": 'success', "message": "Personal details added successfully."}
                    elif formType=="addressDetail":
                        CandidateData.objects.filter(user_id=user_id).update(
                            latitude=latitude,
                            longitude=longitude,
                            address=address,
                    ...
                    elif formType=="qualificationDetails":
                        CandidateData.objects.filter(user_id=user_id).update(
                        ...
                        response = {"status": 'success', "message": "Qualification details added successfully."}
                    elif formType=="uploadDocuments":
                        CandidateData.objects.filter(user_id=user_id).update(
                            resume=resume,
                            profile_pic=profile_pic,
                            salary_slip=salary_slip
                        )
                        response = {"status": 'success', "message": "Personal documents added successfully."}
                    # response = {"status": 'success', "message": first_name + last_name + 'your account registered successfully.'}
            else:
...
def candidate_login(request):
    try:
        if request.method == 'POST':
            data = json.loads(request.body)
            Candidate = CandidateData.objects.get(phone_no=data['phone_no'])
            print(Candidate.password)
            print(data['password'])
            print(check_password(data['password'], Candidate.password))
            if Candidate and check_password(data['password'], Candidate.password):
                token = generate_token(Candidate.user_id, Candidate.phone_no)
                CandidateData.objects.filter(user_id=Candidate.user_id).update( token=token)
                cand_data = CandidateData.objects.filter(user_id=CandidateData.user_id).values()[0]
                request.session['token'] = Candidate.token
                response = {"status": 'success', "message": 'Candidate logged in successfully.','data':cand_data}
            else:
                response = {"status": 'error', "message": 'Invalid Candidate'}
            return JsonResponse(response)
...
def company_verify_otp(request):
    try:
        if request.method == "POST":
            ...
            if user_data.otp_validity > datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"):
                if user_data.otp == otp:
                    CompanyData.objects.filter(company_id=company_id).update(otp="",phone_verified=True,otp_validity="", token=token, updated_at =date)
                    response = {"status": 'success', "message": "OTP verified successfully."}
                    return JsonResponse(response)
                else:
                    response = {"status": 'error', "message": "OTP mismatched"}
...
def company_register(request):
    try:
        if request.method == 'POST':
            ...
            if CompanyData.objects.filter(email=email).exists() or CompanyData.objects.filter(phone_no=request.POST['phone_no']).exists():
                ...
                if CompanyData.objects.filter(company_name=company_name).exists():
                    ...
                    response = {"status":200, "data":phone_no, "message":"OTP sent to your phone number "+ str(phone_no)+"."}
            return JsonResponse(response)
    except Exception as e:
        traceback.print_exc()
        response = {"status": 'error', "message": f'{str(e)}'}
    ...
    try:
        if request.method == 'POST':
            ...
            if Company and check_password(data['password'], Company.password):
                ...
                response = {"status": 'success', "message": 'Company logged in successfully'}
            else:
    ...
    try:
        if request.method == 'POST':
            ...
            if decoded_token['user_id']:
                if decoded_token['exp_time'] > datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") and CompanyData.objects.filter(company_id=decoded_token['user_id'] , phone_verified=True).exists():
                    candidate_data = CandidateData.objects.all().values()
                    if candidate_data.count() !=0:
                            response= {"status":"success", "message":"Data retrieverd successfully.", "data":list(candidate_data), "count":candidate_data.count()}
                    else:
